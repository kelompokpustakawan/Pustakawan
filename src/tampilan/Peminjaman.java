...
public class Peminjaman extends javax.swing.JFrame {
    ...
    // 2. Reset / Mengaktifkan Form Kosong
    protected void aktif() {
        ...
        txtbuku.setText("");
        txtcari.setText("");
        jComboBox1.setSelectedIndex(0); // Default: "Dipinjam"
        
        // Reset tanggal JCalendarPanel ke waktu sekarang
        jCalendarPanel1.setDate(new Date());
        txtpetugas.requestFocus();
    }

    // 3. Menampilkan Data Buku dari Database ke JTable kanan
    ...
    try {
        // Validasi ketersediaan stok buku sebelum transaksi dilakukan
        ...
        // Memulai transaksi database aman (Transaction Management)
        conn.setAutoCommit(false);

        // 1. Ambil tanggal dari jCalendarPanel1 sebagai tanggal pinjam
        java.util.Date tglPinjamDate = jCalendarPanel1.getDate();

        // 2. LOGIKA TAMBAHAN: Menghitung masa pinjam 7 hari ke depan otomatis
        java.util.Calendar cal = java.util.Calendar.getInstance();
        cal.setTime(tglPinjamDate);
        cal.add(java.util.Calendar.DAY_OF_MONTH, 7); // Menambah durasi 7 hari
        java.util.Date tglKembaliDate = cal.getTime();

        // 3. Format kedua tanggal ke tipe data String SQL (yyyy-MM-dd)
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        String tglPinjam = sdf.format(tglPinjamDate);
        String tglKembali = sdf.format(tglKembaliDate); // Hasil penambahan 7 hari
        ...
        psMaster.setString(3, tglKembali); // Memasukkan data tanggal_kembali (Batas pengembalian)
        ...
        aktif();
        tampilDataBuku(); // Refresh tabel stok buku sebelah kanan

    } catch (SQLException e) {
        try {
            conn.rollback(); // Batalkan data jika ada salah satu proses SQL yang gagal
